...
impl ColumnSpec {
    ...
    pub(crate) fn to_sql(&self, attach_pk_column_to_column_stmt: bool) -> SchemaStatement {
        let SchemaStatement {
            statement,
            post_statements,
            ..
...
pub fn column_default_from_sql(
    default_value: String,
    db_type: Option<&dyn PhysicalColumnType>,
) -> Result<ColumnDefault, DatabaseError> {
    match db_type {
        Some(physical_type) => handle_typed_default(&default_value, physical_type),
...
fn handle_typed_default(
    ...
    if any_type.is::<StringColumnType>() {
        handle_string_default(default_value)
    } else if any_type.is::<UuidColumnType>() {
        handle_uuid_default(default_value)
    } else if any_type.is::<IntColumnType>() {
        handle_int_default(default_value)
    } else if any_type.is::<FloatColumnType>() {
        handle_float_default(default_value)
    } else if any_type.is::<NumericColumnType>() {
        handle_decimal_default(default_value)
    } else if any_type.is::<BooleanColumnType>() {
        handle_boolean_default(default_value)
    } else if any_type.is::<DateColumnType>() {
        handle_date_default(default_value)
    } else if any_type.is::<TimeColumnType>() {
        handle_time_default(default_value)
    } else if any_type.is::<TimestampColumnType>() {
        handle_timestamp_default(default_value)
    } else if any_type.is::<JsonColumnType>() {
        handle_json_default(default_value)
        ...
        handle_enum_default(default_value, enum_type)
    } else {
        Ok(ColumnDefault::Function(default_value.to_string()))
    }
}
...
fn handle_uuid_default(default_value: &str) -> Result<ColumnDefault, DatabaseError> {
    if default_value == "gen_random_uuid()" {
        Ok(ColumnDefault::Uuid(UuidGenerationMethod::Random))
    } else if default_value == "uuid_generate_v4()" {
        Ok(ColumnDefault::Uuid(UuidGenerationMethod::V4))
    } else if default_value == "uuidv7()" {
        Ok(ColumnDefault::Uuid(UuidGenerationMethod::V7))
    } else {
    ...
    match s_upper.find('[') {
        ...
        None => {
            ...
            if let Some(enum_type) = enum_type {
                ...
                exo_sql_pg::physical_column_type::physical_column_type_from_string(s)
            }
        }
    }
}
