...
use exo_sql_pg::physical_column_type::PhysicalColumnTypeExt;
...
use crate::constraint::{Constraints, sorted_comma_list};
use crate::enum_spec::EnumSpec;
use crate::index_spec::IndexSpec;
use crate::issue::WithIssues;
use crate::op::SchemaOp;
use crate::trigger_spec::TriggerSpec;
...
impl TableSpec {
    ...
    pub(crate) async fn from_live_db_table(
        ...
        for foreign_constraint in constraints.foreign_constraints.into_iter() {
            for column_pair in foreign_constraint.column_pairs.into_iter() {
                let crate::constraint::ForeignKeyConstraintColumnPair {
                    self_column,
                    foreign_column,
                } = column_pair;
    ...
    pub(crate) async fn from_live_db_materialized_view(
        client: &DatabaseClient,
        table_name: SchemaObjectName,
        enums: &Vec<EnumSpec>,
    ) -> Result<WithIssues<TableSpec>, DatabaseError> {
    ...
    pub fn get_required_extensions(&self) -> HashSet<String> {
        ...
        for col_spec in self.columns.iter() {
            ...
            if typ.is::<exo_sql_pg::physical_column_type::UuidColumnType>() {
            ...
            if typ.is::<exo_sql_pg::physical_column_type::VectorColumnType>() {
    ...
    pub(crate) fn creation_sql(&self) -> SchemaStatement {
    ...
    pub(crate) fn deletion_sql(&self) -> SchemaStatement {
