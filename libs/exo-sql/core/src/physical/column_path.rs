...
use crate::{
    database::Database, database::TableId, physical_column::ColumnId, relation::RelationColumnPair,
};
...
impl PhysicalColumnPath {
    ...
    /// Build a `PhysicalColumnPath` from a sequence of `ColumnId`s.
    /// Used in tests to conveniently create multi-hop column paths.
    pub fn from_columns(columns: Vec<ColumnId>, database: &Database) -> Self {
        use crate::physical_column::{get_mto_relation_for_columns, get_otm_relation_for_columns};
