            ...
            Some((actual_head, actual_tail)) => {
                ...
                match actual_boxed_head {
                    Some(actual_boxed_head) => {
                        ...
                        assert_eq!(
                            &actual_head,
                            &(&$expected_param as &dyn $crate::core::sql_param::SQLParam),
                            "Parameter mismatch"
                        );
                    },
                    None => {
                        assert_eq!(actual_head.param.as_ref(), (&$expected_param as &dyn $crate::core::sql_param::SQLParam), "Parameter mismatch");
                    }
                }
    ...
    ($actual_params:expr, $expected_param:expr, $($rest:expr), *) => {
        match $actual_params.split_first() {
            Some((actual_head, actual_tail)) => {
                ...
                $crate::assert_params!(actual_tail, $($rest), *);
            },
            None => assert!(false)
        }
    };
...
macro_rules! assert_binding {
    ($actual:expr, $expected_stmt:expr) => {
        ...
        $crate::assert_params!(actual_params);
    };
    ($actual:expr, $expected_stmt:expr, $($rest:expr), *) => {
        let (actual_stmt, actual_params) = $actual;
        assert_eq!(actual_stmt, $expected_stmt);
        $crate::assert_params!(actual_params, $($rest), *);
    };
}
