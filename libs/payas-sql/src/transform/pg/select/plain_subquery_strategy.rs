...
///
/// We use a subselect in which that we can use those clauses (note that compared to the
/// [`PlainJoinStrategy`] case, we wouldn't be able to simply add an "order by", since that doesn't
/// make sense when the return value is an aggregate (like a `json_agg`)).
///
...
/// /// ```sql
/// SELECT COALESCE(...)::text FROM (
///     SELECT "concerts".* FROM "concerts" LEFT JOIN "venues" on "concerts"."venue_id" = "venue"."id" WHERE "concerts"."id" > $1 ORDER BY "venues"."name" LIMIT $2 OFFSET $3
/// ) AS "concerts"
///
