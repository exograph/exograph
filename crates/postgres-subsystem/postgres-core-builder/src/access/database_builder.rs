...
// Re-export parent_predicate from postgres_core_model where it now lives.
// The builder wraps it to convert the error type.
pub fn parent_predicate(
    expr: AccessPredicateExpression<DatabaseAccessPrimitiveExpression>,
    parent_entity: &EntityType,
) -> Result<AccessPredicateExpression<DatabaseAccessPrimitiveExpression>, ModelBuildingError> {
    postgres_core_model::access::parent_predicate(expr, parent_entity)
        .map_err(ModelBuildingError::Generic)
}
