...
mod tests {
    ...
    async fn test_system() -> TestSystem {
        ...
        let published_column_path = PhysicalColumnPath {
            path: vec![ColumnPathLink::Leaf(published_column_id)],
        };

        let owner_id_column_path = PhysicalColumnPath {
            path: vec![ColumnPathLink::Leaf(owner_id_column_id)],
        };

        let dept1_id_column_path = PhysicalColumnPath {
            path: vec![ColumnPathLink::Leaf(dept1_id_column_id)],
        };

        let dept2_id_column_path = PhysicalColumnPath {
            path: vec![ColumnPathLink::Leaf(dept2_id_column_id)],
        };
