...
fn build_create_mutation(
    ...
    let data_param_type_name = type_builder::input_creation_type_name(&ast_type.UNSAFE_name());
    ...
    Mutation {
        name: format!("create{}", &ast_type.UNSAFE_name()),
...
fn build_delete_mutations(
    ...
    let by_pk_delete = Mutation {
        name: format!("delete{}", &ast_type.UNSAFE_name()),
    ...
    let by_predicate_delete = Mutation {
        name: format!("delete{}s", &ast_type.UNSAFE_name()),
...
fn build_update_mutations(
    ...
    let data_param_type_name = type_builder::input_update_type_name(&ast_type.UNSAFE_name());
    let data_param_type_id = building
        .mutation_types
        .get_id(&data_param_type_name)
        .unwrap();
    ...
    let by_pk_update = Mutation {
        name: format!("update{}", &ast_type.UNSAFE_name()),
    ...
    let by_predicate_update = Mutation {
        name: format!("update{}s", &ast_type.UNSAFE_name()),
        kind: MutationKind::Update {
            data_param: MutationDataParameter {
                name: "data".to_string(),
                type_name: data_param_type_name,
